...
(c) Use the `select()` function to select variables `hp` and `mpg`. And plot `mpg` vs `hp`.

- Remark: you could either use base R `plot` or `ggplot`. 
- Hint: `hp` is the x-axis and `mpg` is the y-axis.



(d) Filter all the observations with `mpg` less than `mean(mpg)`. How many observations are there?



(e) From (d), what is the correlation between `mpg` and `disp_per_cyl` using the `summarize` function.
...
(h) By grouping the observations by different values `cyl`, compute the $z$-scores for `mpg` by subtracting the corresponding group mean and dividing the corresponding group standard deviation. Hint: you could use the `scale` function.
...
(j) Do auto-transmission cars have smaller mean values of `mpg`? Compute the sample means and compare. Are the different statistical significant? State clearly the statistical assumptions that you have made.
